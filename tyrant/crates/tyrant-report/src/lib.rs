...
use tyrant_match::{summarize, Matched, Violation};
use serde::Serialize;
...
fn render_agent(violations: &[Violation]) -> String {
    ...
    if violations.is_empty() {
        out.push_str("tyrant: no violations.\n");
        return out;
    }
    for v in violations {
        let _ = writeln!(
    ...
    let _ = writeln!(
        out,
        "tyrant: {} error(s), {} warning(s). Fix the errors above before committing.",
        errors, warnings
    );
    out
}
...
mod tests {
    ...
    use tyrant_token::Span;
    use tyrant_rules::Severity;
