//! `tyrant` command-line interface (proposal §10.3).

use anyhow::{bail, Context, Result};
use camino::{Utf8Path, Utf8PathBuf};
use clap::{Parser, Subcommand, ValueEnum};
use tyrant_core::Project;
use tyrant_frontends::{frontend_for, frontends, SourceFile};
use tyrant_match::summarize;
use tyrant_report::{render, Format as ReportFormat};
use tyrant_rules::{load, CompiledRule, RuleFile};

/// The bundled starter policy packs written by `tyrant init`.
const EXAMPLE_RULES_YAML: &str = include_str!("../assets/tyrant.yaml");
const EXAMPLE_RULES_DSL: &str = include_str!("../assets/tyrant.rules");
const CONFIG_NAME: &str = "tyrant.yaml";
...
enum Command {
    ...
    Check {
        ...
        #[arg(default_value = ".")]
        root: Utf8PathBuf,
        /// Path to the rules file (defaults to <root>/tyrant.yaml).
        #[arg(long)]
    ...
    Init {
        #[arg(default_value = ".")]
        root: Utf8PathBuf,
        /// Output format: `yaml` (tyrant.yaml) or `rules` (textual DSL, tyrant.rules).
...
fn load_rules(
    ...
    let path = match rules {
        Some(p) => p,
        None => {
            // Prefer tyrant.yaml, fall back to the textual tyrant.rules.
            let yaml = root.join(CONFIG_NAME);
            let dsl = root.join("tyrant.rules");
    ...
    if !path.exists() {
        bail!("no rules file at {path}. Run `tyrant init` or pass --rules <file>.");
    }
    ...
    let (compiled, file) = if is_yaml {
        ...
        tyrant_rules::dsl::load(&text).map_err(|e| anyhow::anyhow!("{path}: {e}"))?
    };
    Ok((compiled, file))
}
...
fn cmd_init(root: Utf8PathBuf, format: InitFormat) -> Result<i32> {
    let (filename, content, is_dsl) = match format {
        InitFormat::Yaml => ("tyrant.yaml", EXAMPLE_RULES_YAML, false),
        InitFormat::Rules => ("tyrant.rules", EXAMPLE_RULES_DSL, true),
    };
